  ...
  <body class="bg-slate-50 font-sans antialiased">
      ...
      <main class="flex-1 p-5 lg:p-8 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
          ...
          <div class="bg-gradient-to-br from-amber-600 to-amber-700 text-white rounded-2xl p-6 shadow-md">
            <p class="text-sm opacity-90 flex items-center gap-1"><i class="fas fa-credit-card"></i> available balance</p>
                    <p class="text-3xl font-bold mt-2" id="currentBalance">₱<?php echo number_format($currentBalance, 2); ?></p>
                    
                    <?php if ($pendingBalance > 0): ?>
                          <p class="text-xs opacity-80 mt-1 flex items-center gap-1" id="pendingBalance">
                              <i class="fas fa-clock"></i> ₱<?php echo number_format($pendingBalance, 2); ?> pending approval
                          </p>
                    <?php endif; ?>
                    
                    <p class="text-xs opacity-80 mt-1" id="balanceMessage">
                        <?php echo $currentBalance > 0 ? 'ready for payment' : 'no outstanding balance'; ?>
                    </p>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-5">
                    <h3 class="font-semibold flex items-center gap-2"><i class="fas fa-credit-card text-amber-600"></i> payment methods</h3>
                    ...
                    <button onclick="openAddPaymentModal()"
                        class="text-amber-700 text-sm mt-3 flex items-center gap-1 hover:underline"><i class="fas fa-plus"></i> add new</button>
                </div>
                
                <!-- recent transaction summary -->
                <div class="bg-white rounded-2xl border border-slate-200 p-5">
                    <h3 class="font-semibold flex items-center gap-2"><i class="fas fa-clock-rotate-left text-amber-600"></i> this month</h3>
                    ...
                    <p class="text-xs text-slate-500" id="monthlyCount">
                        total spent · <?php echo $monthlyStats['count']; ?> transaction<?php echo $monthlyStats['count'] != 1 ? 's' : ''; ?>
                    </p>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-8">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-semibold text-lg flex items-center gap-2"><i class="fas fa-credit-card text-amber-600"></i> your payment methods</h2>
                    <button onclick="openAddPaymentModal()"
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-semibold text-lg flex items-center gap-2"><i class="fas fa-rectangle-list text-amber-600"></i> recent transactions</h2>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody id="transactionsTable" class="divide-y">
                            ...
                            <?php else: ?>
                                  <?php foreach (array_slice($payments, 0, 5) as $transaction): ?>
                                        <tr>
                                            <td class="py-3">
                                            ...
                                            <td>
                                                <?php
                                                if ($transaction['booking_type'] == 'hotel') {
                                                  echo 'Hotel Booking';
                                                } elseif ($transaction['booking_type'] == 'restaurant') {
                                                  echo 'Restaurant Reservation';
                                                } else {
                                                  echo 'Payment';
                                                }
                                                ?>
                                            ...
                                            <td>
                                                ...
                                                if ($transaction['approval_status'] == 'approved' && $transaction['payment_status'] == 'completed') {
                                            ...
                                            <td>
                                                <button onclick="viewReceipt(<?php echo $transaction['id']; ?>)"
                                                    class="text-amber-700 text-xs hover:underline">receipt</button>
                                            </td>
                                        </tr>
                                  <?php endforeach; ?>
                            <?php endif; ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-8">
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-amber-600 mt-1">Points added by admin after approval</p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
                    ...
                    <form id="paymentMethodForm" onsubmit="addPaymentMethod(event)">
                        ...
                        <div class="mb-4" id="cardNumberField">
                            <label class="block text-sm font-medium text-slate-700 mb-2" id="cardNumberLabel">Card Number / Account</label>
                        ...
                        <div class="flex gap-3">
                            ...
                            <button type="submit"
                                class="flex-1 bg-amber-600 hover:bg-amber-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium transition">Add Method</button>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <form id="paymentForm" onsubmit="processPayment(event)">
                        ...
                        <div class="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                            <p class="text-xs text-amber-700 flex items-center gap-1">
                                ...
                                Your payment will be pending admin approval and will not be immediately deducted from your balance.
                            </p>
                        </div>
                        ...
                        <div class="flex gap-3">
                            <button type="button" onclick="closePaymentModal()"
                                class="flex-1 border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-50 transition">Cancel</button>
                            <button type="submit"
                                class="flex-1 bg-amber-600 hover:bg-amber-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium transition">Submit Payment</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        function updateBalanceMessage() {
            ...
            if (balanceMessage) {
                if (currentBalance > 0) {
                    balanceMessage.textContent = 'ready for payment';
                    if (payNowBtn) {
        ...
        function updatePaymentsTable() {
            ...
            recentPayments.forEach(payment => {
                ...
                if (payment.approval_status === 'approved' && payment.payment_status === 'completed') {
                    statusClass = 'bg-green-100 text-green-700';
                    statusText = 'completed';
                ...
                const description = payment.booking_type === 'hotel' ? 'Hotel Booking' : 
                                   (payment.booking_type === 'restaurant' ? 'Restaurant Reservation' : 'Payment');
        ...
        window.processPayment = async function (event) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Update local balance - available balance remains the same until admin approval
                    // Only pending balance increases
        ...
        // View Receipt with approval status
        window.viewReceipt = function (paymentId) {
            ...
            // Get points from the payment if available
            const pointsEarned = payment.points_earned || Math.floor(payment.amount / 100) * 5;
            ...
            if (payment.approval_status === 'approved' && payment.payment_status === 'completed') {
                ...
                approvalNote = `
                    <div class="mt-3 pt-2 border-t border-green-200 bg-green-50 p-2 rounded-lg">
                        ...
                        <p class="text-xs text-amber-600 mt-1">
                            ...
                            You'll earn +${pointsEarned} loyalty points (admin will add soon)
                        </p>
                    </div>
                `;
            } else if (payment.approval_status === 'pending') {
                ...
                approvalNote = `
                    <div class="mt-3 pt-2 border-t border-amber-200 bg-amber-50 p-2 rounded-lg">
                        <p class="text-xs text-amber-700 flex items-center gap-1">
                            ...
                            <strong>Pending Approval:</strong> Your payment is being verified by admin
                        </p>
                        <p class="text-xs text-amber-600 mt-1">
                ...
                approvalNote = `
                    <div class="mt-3 pt-2 border-t border-red-200 bg-red-50 p-2 rounded-lg">
            ...
            receiptContent.innerHTML = `
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span>${payment.booking_type == 'hotel' ? 'Hotel Booking' : (payment.booking_type == 'restaurant' ? 'Restaurant Reservation' : 'Payment')}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between text-xs">
                        ...
                        <span class="${statusColor} font-medium">${statusMessage}</span>
                    </div>
                    ${approvalNote}
                </div>
            `;
        ...
        window.viewAllTransactions = function () {
            ...
            // You can implement a full transactions view here
        ...
        function updateUI() {
            ...
            if (pendingBalanceEl) {
                if (pendingBalance > 0) {
                    pendingBalanceEl.innerHTML = `<i class="fas fa-clock"></i> ₱${pendingBalance.toFixed(2)} pending approval`;
                } else {
                    pendingBalanceEl.remove();
                }
            }
