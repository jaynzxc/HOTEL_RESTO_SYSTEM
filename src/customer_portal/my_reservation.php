...
<head>
  ...
  <style>
    ...
    .reservation-card { transition: all 0.15s; }
    .new-reservation-highlight {
      animation: highlightPulse 2s ease-in-out;
    }
    @keyframes highlightPulse {
      0% { background-color: rgba(245, 158, 11, 0.1); border-color: #f59e0b; }
      50% { background-color: rgba(245, 158, 11, 0.3); border-color: #d97706; }
      100% { background-color: rgba(245, 158, 11, 0.1); border-color: #f59e0b; }
    }
  </style>
</head>
  ...
  <div class="min-h-screen flex flex-col lg:flex-row">
    ...
    <aside class="lg:w-80 bg-white border-r border-slate-200 shadow-sm shrink-0">
      ...
      <nav class="p-4 space-y-1.5 text-sm">
        ...
        <a href="./my_reservation.html" class="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-amber-50 text-amber-800 font-medium"><i class="fa-regular fa-calendar-check w-5 text-amber-600"></i>My Reservations</a>
    ...
    <main class="flex-1 p-5 lg:p-8 overflow-y-auto">
      ...
      <div class="flex gap-2 border-b border-slate-200 mb-6" id="tabContainer">
        <button class="tab-btn px-5 py-2 font-medium transition" data-tab="upcoming" id="tabUpcoming">Upcoming (<span id="upcomingCount">0</span>)</button>
        <button class="tab-btn px-5 py-2 text-slate-500 hover:text-slate-700 transition" data-tab="past" id="tabPast">Past (<span id="pastCount">0</span>)</button>
        <button class="tab-btn px-5 py-2 text-slate-500 hover:text-slate-700 transition" data-tab="cancelled" id="tabCancelled">Cancelled (<span id="cancelledCount">0</span>)</button>
      </div>
      ...
      <div class="mt-8 bg-amber-50 border border-amber-200 rounded-2xl p-5 flex flex-wrap items-center justify-between gap-4">
        ...
        <button class="bg-white border border-amber-600 text-amber-700 px-5 py-2 rounded-xl text-sm hover:bg-amber-50" id="contactSupportBtn">contact support</button>
      </div>
        ...
        if (list.length === 0) {
          container.innerHTML = `
            <div class="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400">
              ...
              ${activeTab === 'upcoming' ? `
                <div class="mt-6">
                  <a href="./hotel_booking.html" class="inline-block bg-amber-600 hover:bg-amber-700 text-white px-6 py-3 rounded-xl text-sm font-medium transition mx-2">
                    <i class="fa-solid fa-hotel mr-2"></i>Book Hotel
                  </a>
                  <a href="./restaurant_reservation.html" class="inline-block border border-amber-600 text-amber-700 hover:bg-amber-50 px-6 py-3 rounded-xl text-sm font-medium transition mx-2">
