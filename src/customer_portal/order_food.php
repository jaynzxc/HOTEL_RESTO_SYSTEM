...
<body class="bg-slate-50 font-sans antialiased">
        ...
        <main class="flex-1 p-5 lg:p-8 overflow-y-auto">
            ...
            <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="bg-white border border-slate-200 rounded-full px-4 py-2 text-sm flex items-center gap-2 shadow-sm">
                    <i class="fas fa-calendar text-slate-400"></i> <?php echo date('l, d F Y'); ?>
                </div>
            </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="menuGrid">
                        <?php foreach ($menuItems as $item): ?>
                              <div class="menu-item bg-white rounded-2xl border border-slate-200 p-4 flex gap-3 hover:shadow-md transition"
                ...
                <div class="space-y-5">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-5 sticky top-4">
                        <h2 class="font-semibold text-lg flex items-center gap-2 border-b pb-3">
                            <i class="fa-solid fa-bag-shopping text-amber-600"></i> your order
                        </h2>
                        ...
                        <div class="mt-4 pt-3 border-t border-slate-200">
                            ...
                            <div class="flex gap-2">
                                <input type="text" id="foodPromoCodeInput" placeholder="Enter promo code" 
                                    class="flex-1 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-1 focus:ring-amber-500 outline-none">
                                <button onclick="applyFoodPromoCode()" id="applyFoodPromoBtn" 
                                    class="bg-amber-600 text-white px-4 py-2 rounded-xl text-sm hover:bg-amber-700 whitespace-nowrap">
                                    Apply
                                </button>
                            </div>
                            <div id="foodPromoMessage" class="text-xs mt-2 hidden"></div>
                            
                            <!-- Discount Display -->
                            <div id="foodDiscountDisplay" class="mt-3 hidden bg-green-50 p-3 rounded-lg">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <i class="fas fa-tag text-green-600 mr-1"></i>
                                        <span class="text-sm font-medium text-green-700">Promo Applied!</span>
                                        <p class="text-xs text-green-600" id="foodPromoName"></p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-sm font-semibold text-green-700">-₱<span id="foodDiscountAmount">0</span></p>
                                        <button onclick="removeFoodPromoCode()" class="text-xs text-red-500 hover:text-red-700">Remove</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <button id="placeOrderBtn"
                            class="w-full bg-amber-600 hover:bg-amber-700 text-white py-3 rounded-xl font-medium mt-5 transition">
                            place order
                        </button>
    ...
    <script>
        (function () {
            ...
            let appliedFoodPromo = null;
            let foodPromoDiscount = 0;
            let foodPromoCodeId = null;
            ...
            function showToast(message, type = 'success') {
                ...
                const bgColor = type === 'success' ? 'border-green-500' : type === 'error' ? 'border-red-500' : 'border-amber-500';
                const iconColor = type === 'success' ? 'text-green-600' : type === 'error' ? 'text-red-600' : 'text-amber-600';
                const iconBg = type === 'success' ? 'bg-green-100' : type === 'error' ? 'bg-red-100' : 'bg-amber-100';
                const icon = type === 'success' ? 'fa-check-circle' : type === 'error' ? 'fa-exclamation-circle' : 'fa-info-circle';
                
                toast.className = `fixed bottom-4 right-4 bg-white rounded-lg shadow-lg p-4 transform translate-x-full transition-transform duration-300 z-50 border-l-4 ${bgColor}`;
                toast.innerHTML = `
                    <div class="flex items-center gap-3">
                        <div class="h-8 w-8 rounded-full ${iconBg} flex items-center justify-center">
            ...
            function updateFoodCartSummary() {
                ...
                let discountHtml = '';
                if (foodPromoDiscount > 0 && appliedFoodPromo) {
                    total = total - foodPromoDiscount;
                    discountHtml = `
                        <div class="flex justify-between items-center text-green-600">
                            <span>promo discount (${appliedFoodPromo.promo_code})</span><span>-₱${foodPromoDiscount.toFixed(2)}</span>
                        </div>
                    `;
                }
                ...
                summaryDetails.innerHTML = `
                    ...
                    <div class="flex justify-between items-center text-xs text-green-600 mt-2">
            ...
            function renderOrder() {
                ...
                if (order.length === 0) {
                    ...
                    document.getElementById('foodDiscountDisplay')?.classList.add('hidden');
                    return;
                }
            ...
            window.applyFoodPromoCode = function() {
                const promoCode = document.getElementById('foodPromoCodeInput').value.trim().toUpperCase();
                if (!promoCode) {
                ...
                fetch('../../controller/customer/post/validate_food_promo.php', {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        appliedFoodPromo = data;
                        foodPromoDiscount = data.discount;
                        foodPromoCodeId = data.promo_code_id;
                        
                        const discountDisplay = document.getElementById('foodDiscountDisplay');
                        discountDisplay.classList.remove('hidden');
                        document.getElementById('foodDiscountAmount').innerText = data.discount.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                        });
                        document.getElementById('foodPromoName').innerHTML = `<i class="fas fa-tag"></i> ${data.promo_code} - ${data.discount_type === 'percentage' ? data.discount_value + '% off' : '₱' + data.discount_value.toLocaleString() + ' off'}`;
                        
                        const promoMessage = document.getElementById('foodPromoMessage');
            ...
            window.removeFoodPromoCode = function() {
                appliedFoodPromo = null;
                foodPromoDiscount = 0;
                foodPromoCodeId = null;
                
                updateFoodCartSummary();
                
                document.getElementById('foodDiscountDisplay').classList.add('hidden');
                document.getElementById('foodPromoMessage').classList.add('hidden');
                document.getElementById('foodPromoCodeInput').value = '';
            ...
            if (placeOrderBtn) {
                placeOrderBtn.addEventListener('click', () => {
                    ...
                    const subtotal = order.reduce((acc, i) => acc + (i.isFree ? 0 : i.price * i.quantity), 0);
                    const service = Math.round(subtotal * 0.05 * 100) / 100;
                    let finalTotal = subtotal + service;
                    
                    if (foodPromoDiscount > 0 && appliedFoodPromo) {
                        finalTotal = finalTotal - foodPromoDiscount;
                    }
                    ...
                    if (foodPromoCodeId && foodPromoDiscount > 0) {
                        orderData.promo_code_id = foodPromoCodeId;
                        orderData.discount_applied = foodPromoDiscount;
                    }
                    ...
                    if (orderData.promo_code_id) {
                        params.append('promo_code_id', orderData.promo_code_id);
                        params.append('discount_applied', orderData.discount_applied);
                    }
                    ...
                    .then(data => {
                        if (data.success) {
                            const pointsEarned = Math.floor(subtotal / 100) * 5;
                            const discountHtml = foodPromoDiscount > 0 ? `<p><strong>Promo Discount:</strong> -₱${foodPromoDiscount.toFixed(2)}</p>` : '';
                    ...
                    html: `
                        <div class="text-left">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-xs">
                                <div><span class="text-slate-500">Status:</span> <span class="ml-1 font-medium">${order.status}</span></div>
                                <div><span class="text-slate-500">Type:</span> <span class="ml-1 font-medium">${order.order_type}</span></div>
                                <div><span class="text-slate-500">Points used:</span> <span class="ml-1 font-medium">${order.points_used || 0}</span></div>
                                <div><span class="text-slate-500">Points earned:</span> <span class="ml-1 font-medium text-green-600">+${order.points_earned || 0}</span></div>
                            </div>
                        </div>
                    `,
                    confirmButtonColor: '#d97706'
