...
<head>
  ...
  <style>
    .waiting-pulse { animation: softPulse 2s infinite; }
    @keyframes softPulse { 0% { opacity: 0.7; } 50% { opacity: 1; } 100% { opacity: 0.7; } }
    .table-tile { transition: all 0.1s ease; user-select: none; cursor: pointer; }
    .table-tile:hover { box-shadow: 0 4px 8px rgba(0,0,0,0.05); }
...
<div class="min-h-screen flex flex-col lg:flex-row">
  ...
  <aside class="lg:w-80 bg-white border-r border-slate-200 shadow-sm flex-shrink-0">
    ...
    <div class="flex items-center gap-3 px-6 py-5 border-b border-slate-100 bg-slate-50/80">
      <div class="h-12 w-12 rounded-full bg-amber-200 flex items-center justify-center text-amber-800 font-bold text-lg">JM</div>
      <div>
        <p class="font-medium text-slate-800">J. Mateo <span class="text-xs bg-amber-100 px-2 py-0.5 rounded-full">maître d'</span></p>
        <p class="text-xs text-slate-500 flex items-center gap-1"><i class="fa-regular fa-bell text-[11px]"></i> <span id="sidebarNotificationCount">0</span> notifications</p>
      </div>
    </div>
    ...
    <!-- navigation (restaurant reservation active) -->
     <nav class="p-4 space-y-1.5 text-sm">
      ...
      <a href="./restaurant_reservation.html" class="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-amber-50 text-amber-800 font-medium"><i class="fa-solid fa-clock w-5 text-amber-600"></i>Restaurant Reservation</a>
  ...
  <main class="flex-1 p-5 lg:p-8 overflow-y-auto" id="mainContent">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="bg-white border border-slate-200 rounded-full px-4 py-2 text-sm flex items-center gap-2 shadow-sm">
        <i class="fa-regular fa-calendar text-slate-400"></i> wednesday, 21 may 2025 · <span class="text-amber-600 font-medium">service: 5:00 PM</span>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div class="xl:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label class="block text-xs text-slate-500 mb-1">customer name <span class="text-red-400">*</span></label>
              <input type="text" id="resName" placeholder="e.g. Mia Cruz" class="w-full border border-slate-200 rounded-xl p-3 text-sm bg-white" value="">
            </div>
            <div>
              <label class="block text-xs text-slate-500 mb-1">contact number <span class="text-red-400">*</span></label>
              <input type="tel" id="resPhone" placeholder="+63 912 345 6789" class="w-full border border-slate-200 rounded-xl p-3 text-sm bg-white" value="">
            </div>
            <div>
              <label class="block text-xs text-slate-500 mb-1">email address <span class="text-red-400">*</span></label>
              <input type="email" id="resEmail" placeholder="guest@example.com" class="w-full border border-slate-200 rounded-xl p-3 text-sm bg-white" value="">
            </div>
            <div>
              <label class="block text-xs text-slate-500 mb-1">number of guests <span class="text-red-400">*</span></label>
            ...
            <div>
              <label class="block text-xs text-slate-500 mb-1">reservation date <span class="text-red-400">*</span></label>
              <input type="date" id="resDate" value="2025-05-21" class="w-full border border-slate-200 rounded-xl p-3 text-sm">
            </div>
            <div>
              <label class="block text-xs text-slate-500 mb-1">reservation time <span class="text-red-400">*</span></label>
            ...
            <div class="md:col-span-2">
              ...
              <textarea id="resRequest" rows="2" placeholder="allergies, celebration, etc." class="w-full border border-slate-200 rounded-xl p-3 text-sm"></textarea>
            </div>
          </div>
          ...
          <div class="mt-5 p-4 bg-amber-50 rounded-xl border border-amber-200">
            <div class="flex items-center justify-between mb-2">
              <p class="font-medium flex items-center gap-2"><i class="fa-solid fa-coins text-amber-600"></i> Down Payment Calculation</p>
            ...
            <p class="text-xs text-slate-500 mt-2"><i class="fa-regular fa-circle-info"></i> 50% of down payment will be converted to loyalty points after visit</p>
          </div>

          <div class="flex flex-wrap items-center gap-3 mt-6">
            <button class="bg-amber-600 hover:bg-amber-700 text-white px-8 py-3 rounded-xl font-medium transition" id="submitReservationBtn">
              <i class="fa-regular fa-credit-card mr-2"></i>submit & pay down payment
            </button>
            <button class="border border-slate-300 text-slate-600 hover:bg-slate-50 px-6 py-3 rounded-xl font-medium transition text-sm" id="validateFieldsBtn"><i class="fa-regular fa-circle-check"></i> validate fields</button>
          </div>
          <p class="text-xs text-emerald-600 mt-2 flex items-center gap-1" id="validationHint"><i class="fa-regular fa-circle-check"></i> fill required fields</p>
        </div>

        <!-- 2️⃣ RESERVATION CHECKING + TABLE ASSIGNMENT (dynamic, fully functional) -->
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <h2 class="font-semibold text-xl flex items-center gap-2 mb-3"><i class="fa-regular fa-clipboard text-amber-600"></i> reservation checking & table assignment</h2>
          <div class="flex flex-wrap gap-4 items-center border-b border-slate-100 pb-4">
            <div class="bg-amber-50 px-4 py-2 rounded-xl"><span class="text-sm" id="currentWalkinDisplay">incoming: (no guest) · 0 guests · --:--</span></div>
            <span class="bg-blue-100 text-blue-700 text-xs px-3 py-1 rounded-full"><i class="fa-regular fa-question"></i> walk‑in (no prior res)</span>
          </div>

          <!-- walk-in guest handling -->
          <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="border rounded-xl p-4 bg-slate-50">
              <p class="font-medium flex items-center gap-2"><i class="fa-regular fa-person-walking"></i> walk-in guest</p>
              <p class="text-xs text-slate-500 mt-1">check table availability</p>
              <div class="mt-3 flex items-center gap-2 text-sm" id="availabilityMessage"><i class="fa-regular fa-clock"></i> <span class="text-amber-600">enter guest details first</span></div>
              <button class="mt-3 w-full bg-green-600 hover:bg-green-700 text-white text-sm py-2 rounded-xl transition flex items-center justify-center gap-2" id="assignWalkinBtn"><i class="fa-regular fa-bell"></i> assign & notify waiter</button>
            </div>
            <div class="border rounded-xl p-4 bg-red-50/40">
              <p class="font-medium flex items-center gap-2"><i class="fa-regular fa-timer"></i> if no table available</p>
              <p class="text-xs text-slate-500 mt-1">move to waiting list, show estimated wait.</p>
              <div class="flex items-center gap-2 mt-2 text-sm"><i class="fa-regular fa-hourglass-half"></i> est. wait: 15‑20 min</div>
              <button class="mt-3 w-full bg-slate-600 hover:bg-slate-700 text-white text-sm py-2 rounded-xl transition" id="addToWaitingBtn"><i class="fa-regular fa-list"></i> add to waiting list</button>
            </div>
          </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5">
          <h3 class="font-semibold flex items-center gap-2 mb-3"><i class="fa-regular fa-list-timeline text-amber-600"></i> waiting list queue · <span id="waitingCount">0</span> parties</h3>
          ...
          <div class="flex justify-between items-center mt-4 text-xs text-slate-500 border-t pt-3">
            ...
            <button class="text-amber-700 hover:underline" id="simulateTableFreeBtn"><i class="fa-regular fa-bell"></i> simulate table free</button>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5">
          <h3 class="font-semibold flex items-center gap-2 mb-3"><i class="fa-regular fa-grid-2 text-amber-600"></i> real‑time table status</h3>
          ...
          <div class="flex flex-wrap gap-3 mt-4 text-[10px] text-slate-500 border-t pt-3">
            <span><span class="inline-block w-3 h-3 bg-green-100 border border-green-300 rounded"></span> available</span>
            <span><span class="inline-block w-3 h-3 bg-amber-100 border border-amber-300 rounded"></span> reserved</span>
            <span><span class="inline-block w-3 h-3 bg-red-100 border border-red-300 rounded"></span> occupied</span>
          </div>
          <p class="text-xs text-slate-400 mt-2"><i class="fa-regular fa-comment"></i> click table to toggle status (demo)</p>
        </div>

        <!-- staff notification panel (empty at start) -->
        <div class="bg-white border border-slate-200 rounded-2xl p-5">
          <h4 class="font-medium flex items-center gap-2"><i class="fa-regular fa-bell text-amber-600"></i> recent staff notifications</h4>
          <ul class="text-xs space-y-3 mt-3" id="notificationList">
            <li class="text-slate-400 text-center">No notifications yet.</li>
          </ul>
          <button class="w-full border border-amber-600 text-amber-700 hover:bg-amber-50 mt-4 py-2 rounded-xl text-sm transition" id="refreshNotifications"><i class="fa-regular fa-rotate"></i> refresh notifications</button>
        </div>

        <!-- seating time demo (dynamic seating recorded separately) -->
        <div class="bg-slate-100 border border-slate-300 rounded-2xl p-5">
          <h4 class="font-medium flex items-center gap-2"><i class="fa-regular fa-clock text-amber-600"></i> seating time recording</h4>
...
<div id="paymentProcessingModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    ...
    function processDownPaymentAndRedirect(reservationData) {
      ...
      setTimeout(() => {
        ...
        if (window.location.href.includes('restaurant_reservation')) {
          ...
          console.log('Redirect to payments.html with data:', paymentData);
        }
      }, 2000);
    }
