...
<body class="bg-white font-sans antialiased">
      ...
      <form id="addItemForm" onsubmit="saveNewItem(event)">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="mb-4">
            ...
            <select id="newItemCategory" onchange="generateItemCode()" class="w-full border border-slate-200 rounded-lg px-3 py-2">
          ...
          <div class="mb-4">
            ...
            <input type="text" id="newItemCode" readonly class="w-full border border-slate-200 rounded-lg px-3 py-2 bg-slate-50">
          </div>
          
          <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">Price (₱)</label>
            <input type="number" id="newItemPrice" required min="0" class="w-full border border-slate-200 rounded-lg px-3 py-2">
          </div>
          
          <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">Cost (₱)</label>
            <input type="number" id="newItemCost" required min="0" class="w-full border border-slate-200 rounded-lg px-3 py-2">
          </div>
          
          <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">Stock</label>
            <input type="number" id="newItemStock" required min="0" class="w-full border border-slate-200 rounded-lg px-3 py-2">
          </div>
          ...
          <div class="mb-4 col-span-2">
            ...
            <textarea id="newItemDescription" rows="2" class="w-full border border-slate-200 rounded-lg px-3 py-2"></textarea>
          </div>
        </div>
        
        <div class="flex gap-3 mt-4">
          <button type="submit" class="flex-1 bg-amber-600 text-white py-2 rounded-lg hover:bg-amber-700">Add Item</button>
          <button type="button" onclick="closeModal('addItemModal')" class="flex-1 border border-slate-200 py-2 rounded-lg hover:bg-slate-50">Cancel</button>
        </div>
      </form>
    </div>
  </div>
    ...
    <div class="modal-content p-6">
      ...
      <form id="addCategoryForm" onsubmit="saveNewCategory(event)">
        <div class="mb-4">
          ...
          <input type="text" id="newCategoryName" required class="w-full border border-slate-200 rounded-lg px-3 py-2">
        </div>
        
        <div class="mb-4">
          <label class="block text-sm font-medium text-slate-700 mb-1">Description (optional)</label>
          <textarea id="newCategoryDescription" rows="2" class="w-full border border-slate-200 rounded-lg px-3 py-2"></textarea>
        </div>
        
        <div class="mb-4">
          <label class="block text-sm font-medium text-slate-700 mb-1">Display Order</label>
          <input type="number" id="newCategoryOrder" value="1" min="1" class="w-full border border-slate-200 rounded-lg px-3 py-2">
        </div>
        
        <div class="flex gap-3 mt-4">
          <button type="submit" class="flex-1 bg-amber-600 text-white py-2 rounded-lg hover:bg-amber-700">Add Category</button>
          <button type="button" onclick="closeModal('addCategoryModal')" class="flex-1 border border-slate-200 py-2 rounded-lg hover:bg-slate-50">Cancel</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <!-- Bulk Edit Panel -->
  <div id="bulkEditPanel" class="fixed bottom-0 left-0 right-0 bg-white border-t border-amber-200 shadow-lg p-4 transform translate-y-full transition-transform duration-300 z-50" style="margin-left: 320px;">
    <div class="container mx-auto flex flex-wrap items-center justify-between">
      ...
      <div class="flex gap-3">
        ...
        <input type="number" id="bulkPrice" placeholder="Set Price" class="border border-slate-200 rounded-lg px-3 py-2 text-sm w-32">
        <button onclick="applyBulkEdit()" class="bg-amber-600 text-white px-4 py-2 rounded-lg text-sm">Apply</button>
        <button onclick="cancelBulkEdit()" class="border border-slate-200 px-4 py-2 rounded-lg text-sm">Cancel</button>
      </div>
    </div>
  </div>
  ...
  <div class="min-h-screen flex flex-col lg:flex-row">
    ...
    <aside class="lg:w-80 bg-white border-r border-slate-200 shadow-sm lg:min-h-screen shrink-0 overflow-y-auto">
      ...
      <nav class="p-4 space-y-2 text-sm">
        ...
        <!-- RESTAURANT MANAGEMENT GROUP - open with Menu Management highlighted -->
        <details class="group" open>
          ...
          <div class="ml-6 mt-1 space-y-1 pl-3 border-l-2 border-amber-200">
            ...
            <a href="../restaurant_management/menu_management.html" class="flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-100/50 text-amber-700 font-medium"><i class="fa-solid fa-bars w-4 text-amber-600"></i> Menu Management</a>
    ...
    <main class="flex-1 p-5 lg:p-8 overflow-y-auto bg-white">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 p-4 mb-6 flex flex-wrap gap-3 items-center justify-between">
        <div class="flex gap-2 flex-wrap">
          <button onclick="openModal('addItemModal')" class="bg-amber-600 text-white px-4 py-2 rounded-xl text-sm hover:bg-amber-700 transition">+ add new item</button>
          <button onclick="openModal('addCategoryModal')" class="bg-amber-600 text-white px-4 py-2 rounded-xl text-sm hover:bg-amber-700 transition">+ add category</button>
          <button onclick="toggleBulkEdit()" class="border border-slate-200 px-4 py-2 rounded-xl text-sm hover:bg-slate-50 transition">bulk edit</button>
        </div>
        <div class="relative">
          <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
          <input type="text" id="searchInput" onkeyup="searchMenu()" placeholder="search menu items..." class="border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-sm w-64 focus:ring-1 focus:ring-amber-500 outline-none">
        </div>
      </div>

      <!-- ===== CATEGORY TABS ===== -->
      <div class="flex flex-wrap gap-2 border-b border-slate-200 pb-2 mb-6">
        <button id="catAll" onclick="filterByCategory('all', event)" class="category-filter px-4 py-2 bg-amber-600 text-white rounded-full text-sm">all</button>
        <button id="catAppetizers" onclick="filterByCategory('appetizers', event)" class="category-filter px-4 py-2 bg-white border border-slate-200 rounded-full text-sm hover:bg-slate-50">appetizers</button>
        <button id="catMains" onclick="filterByCategory('mains', event)" class="category-filter px-4 py-2 bg-white border border-slate-200 rounded-full text-sm hover:bg-slate-50">mains</button>
        <button id="catDesserts" onclick="filterByCategory('desserts', event)" class="category-filter px-4 py-2 bg-white border border-slate-200 rounded-full text-sm hover:bg-slate-50">desserts</button>
        <button id="catBeverages" onclick="filterByCategory('beverages', event)" class="category-filter px-4 py-2 bg-white border border-slate-200 rounded-full text-sm hover:bg-slate-50">beverages</button>
        <button id="catSpecials" onclick="filterByCategory('specials', event)" class="category-filter px-4 py-2 bg-white border border-slate-200 rounded-full text-sm hover:bg-slate-50">specials</button>
        <button id="catSides" onclick="filterByCategory('sides', event)" class="category-filter px-4 py-2 bg-white border border-slate-200 rounded-full text-sm hover:bg-slate-50">sides</button>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden mb-8">
          ...
          <table class="w-full text-sm" id="menuTable">
            ...
            <tbody class="divide-y" id="menuTableBody">
              <tr data-category="mains" data-status="available">
                <td class="p-4"><input type="checkbox" class="item-checkbox rounded border-slate-300" onclick="updateSelectedCount()"></td>
                <td class="p-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div><span class="font-medium">Sinigang na Baboy</span><p class="text-xs text-slate-400">#M001</p></div>
                  </div>
                </td>
                ...
                <td class="p-4">₱180</td>
                <td class="p-4">45</td>
                <td class="p-4"><span class="status-badge bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs">available</span></td>
                <td class="p-4">
                  <button class="text-amber-700 text-xs hover:underline mr-2" onclick="editItem(this)">edit</button>
                  <button class="text-rose-600 text-xs hover:underline" onclick="toggleItemStatus(this)">disable</button>
                </td>
              </tr>
              <tr data-category="mains" data-status="available">
                <td class="p-4"><input type="checkbox" class="item-checkbox rounded border-slate-300" onclick="updateSelectedCount()"></td>
                <td class="p-4">
                  <div class="flex items-center gap-3">
                    <div class="h-10 w-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-500"><i class="fa-regular fa-image text-xs"></i></div>
                    <div><span class="font-medium">Sizzling Sisig</span><p class="text-xs text-slate-400">#M002</p></div>
                  </div>
                </td>
                <td class="p-4">Mains</td>
              ...
              <tr data-category="mains" data-status="available">
                ...
                <td class="p-4">
                  <div class="flex items-center gap-3">
                    <div class="h-10 w-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-500"><i class="fa-regular fa-image text-xs"></i></div>
                    <div><span class="font-medium">Crispy Pata</span><p class="text-xs text-slate-400">#M003</p></div>
                  </div>
                </td>
                <td class="p-4">Mains</td>
              ...
              <tr data-category="desserts" data-status="available">
                ...
                <td class="p-4">
                  <div class="flex items-center gap-3">
                    <div class="h-10 w-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-500"><i class="fa-regular fa-image text-xs"></i></div>
                    <div><span class="font-medium">Halo-Halo</span><p class="text-xs text-slate-400">#D001</p></div>
                  </div>
                </td>
              ...
              <tr data-category="beverages" data-status="out of stock">
                ...
                <td class="p-4">
                  <div class="flex items-center gap-3">
                    <div class="h-10 w-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-500"><i class="fa-regular fa-image text-xs"></i></div>
                    <div><span class="font-medium">Fresh Buko Juice</span><p class="text-xs text-slate-400">#B001</p></div>
                  </div>
                </td>
                ...
                <td class="p-4">₱30</td>
                <td class="p-4">0</td>
                <td class="p-4"><span class="status-badge bg-rose-100 text-rose-700 px-2 py-0.5 rounded-full text-xs">out of stock</span></td>
                <td class="p-4">
                  <button class="text-amber-700 text-xs hover:underline mr-2" onclick="editItem(this)">edit</button>
              ...
              <tr data-category="sides" data-status="available">
                ...
                <td class="p-4">
                  <div class="flex items-center gap-3">
                    <div class="h-10 w-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-500"><i class="fa-regular fa-image text-xs"></i></div>
                    <div><span class="font-medium">Garlic Rice</span><p class="text-xs text-slate-400">#S001</p></div>
                  </div>
                </td>
              ...
              <tr data-category="specials" data-status="special">
                ...
                <td class="p-4">
                  <div class="flex items-center gap-3">
                    <div class="h-10 w-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-500"><i class="fa-regular fa-image text-xs"></i></div>
                    <div><span class="font-medium">Seafood Kare-Kare</span><p class="text-xs text-slate-400">#M004</p></div>
                  </div>
                </td>
                ...
                <td class="p-4">₱260</td>
                <td class="p-4">12</td>
                <td class="p-4"><span class="status-badge bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-xs">special</span></td>
                <td class="p-4">
                  <button class="text-amber-700 text-xs hover:underline mr-2" onclick="editItem(this)">edit</button>
                  <button class="text-rose-600 text-xs hover:underline" onclick="toggleItemStatus(this)">disable</button>
                </td>
        ...
        <div class="p-4 border-t border-slate-200 flex items-center justify-between">
          ...
          <div class="flex gap-2" id="paginationButtons">
            ...
            <button onclick="changePage(1)" class="border px-3 py-1 rounded-lg text-sm page-btn bg-amber-600 text-white">1</button>
