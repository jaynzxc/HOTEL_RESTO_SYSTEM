      ...
      <main class="flex-1 p-5 lg:p-8 overflow-y-auto bg-white">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          ...
          <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <p class="text-2xl font-semibold" id="statsRevenue">
              ₱<?php echo number_format($stats['today_revenue'], 2); ?></p>
          </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2 border-b border-slate-200 pb-2 mb-6">
          <button onclick="filterOrders('all')"
            class="filter-btn px-4 py-2 <?php echo $typeFilter == 'all' ? 'bg-amber-600 text-white' : 'bg-white border border-slate-200 hover:bg-slate-50'; ?> rounded-full text-sm">all
            orders</button>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden mb-8">
          <div class="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
            <h2 class="font-semibold flex items-center gap-2"><i
                class="fa-solid fa-cash-register text-amber-600"></i>active orders</h2>
                ...
                <?php else: ?>
                  <?php foreach ($orders as $order):
                    ...
                    <tr data-order-id="<?php echo $order['order_reference']; ?>"
                      data-type="<?php echo $order['order_type']; ?>" data-status="<?php echo $order['status']; ?>">
                      <td class="p-3 font-medium">#<?php echo $order['order_reference']; ?></td>
                      <td class="p-3"><?php echo date('h:i A', strtotime($order['created_at'])); ?></td>
                      <td class="p-3"><span
                          class="type-badge <?php echo $typeColor; ?> px-2 py-0.5 rounded-full text-xs"><?php echo $order['order_type']; ?></span>
                      </td>
                      ...
                      <td class="p-3"><span
                          class="status-badge <?php echo $statusColor; ?> px-2 py-0.5 rounded-full text-xs"><?php echo $order['status']; ?></span>
                      </td>
          ...
          <div class="p-4 border-t border-slate-200 flex items-center justify-between">
            <span class="text-xs text-slate-500" id="paginationInfo">
              Showing
              <?php echo (($currentPage - 1) * $limit + 1); ?>-<?php echo min($currentPage * $limit, $totalOrders); ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-amber-50 border border-amber-200 rounded-2xl p-5">
            <div class="flex justify-between items-center mb-3">
              <h3 class="font-semibold flex items-center gap-2"><i class="fas fa-clock text-amber-600"></i>kitchen queue
              </h3>
            ...
            <ul class="space-y-3" id="kitchenQueue">
              ...
              <?php else: ?>
                <?php foreach ($kitchenQueue as $item):
                  ...
                  ?>
                  <li class="flex justify-between items-center border-b border-amber-200 pb-2 queue-item"
                    data-order-id="<?php echo $item['order_reference']; ?>">
                    <div>
                      <span class="font-medium">#<?php echo $item['order_reference']; ?></span>
                      <p class="text-xs text-slate-500"><?php echo $customerInfo; ?> · <?php echo $itemCount; ?> items ·
                        <?php echo $item['minutes_ago']; ?> min ago
                      </p>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="bg-amber-100 text-amber-700 text-xs px-2 py-0.5 rounded-full">preparing</span>
                      <button onclick="updateOrderStatus('<?php echo $item['order_reference']; ?>', 'ready')"
                        class="text-xs text-green-600 hover:underline">✓ ready</button>
                    </div>
                  </li>
                <?php endforeach; ?>
              <?php endif; ?>
            ...
            <div class="mt-4 pt-3 border-t border-amber-200 flex justify-between text-xs text-amber-700">
              <span><span class="font-semibold" id="queueCount"><?php echo count($kitchenQueue); ?></span> orders in
                kitchen</span>
          ...
          <div class="bg-green-50 border border-green-200 rounded-2xl p-5">
            <h3 class="font-semibold flex items-center gap-2 mb-3"><i class="fas fa-circle-check text-green-600"></i>
              ready for pickup/delivery</h3>
            <ul class="space-y-3" id="readyQueue">
              ...
              <?php else: ?>
                <?php foreach ($readyQueue as $item):
                  ...
                  ?>
                  <li class="flex justify-between items-center border-b border-green-200 pb-2"
                    data-order-id="<?php echo $item['order_reference']; ?>">
                    <div>
                      <span class="font-medium">#<?php echo $item['order_reference']; ?></span>
                    ...
                    <div class="flex items-center gap-2">
                      <span
                        class="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full"><?php echo $item['status']; ?></span>
                      <button
                        onclick="updateOrderStatus('<?php echo $item['order_reference']; ?>', '<?php echo $nextStatus; ?>')"
                        class="text-xs text-blue-600 hover:underline"><?php echo $actionText; ?></button>
                    </div>
                  </li>
                <?php endforeach; ?>
              <?php endif; ?>
            ...
            <div class="mt-4 pt-3 border-t border-green-200">
              <span class="text-xs text-green-700"><span class="font-semibold"
                  id="readyCount"><?php echo count($readyQueue); ?></span> order waiting</span>
            </div>
          </div>
        </div>
      </main>
