<?php
/**
 * POST Controller - Validate Food Promo Code
 * Checks if a promo code is valid for food orders and calculates discount
 */
...
try {
    ...
    $promo = $db->query(
        "SELECT pc.*, c.campaign_name, c.target_audience, c.campaign_type
         FROM promo_codes pc
    ...
    // Check if promo is valid for food orders
    $validTypes = ['discount', 'flash_sale', 'seasonal', NULL];
    if ($promo['campaign_type'] && !in_array($promo['campaign_type'], $validTypes)) {
        echo json_encode([
            'success' => false,
            'message' => 'This promo code is not valid for food orders'
        ]);
        exit();
    }
    ...
    $userUsage = $db->query(
        "SELECT COUNT(*) as count FROM food_orders 
    ...
    $serviceFee = $newSubtotal * 0.05; // 5% service fee
    $newTotal = $newSubtotal + $serviceFee;
    ...
    error_log('Food Promo validation error: ' . $e->getMessage());
    echo json_encode([
        'success' => false,
